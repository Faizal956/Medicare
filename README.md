<![CDATA[# 💊 Medi-Remind — AI-Powered Health Assistant
...
| ⚠️ **Drug Interaction Checker** | Automatically checks new medicines against the user's current medication list for dangerous interactions |
...
| ☁️ **Cloud Sync** | All data (profiles, medications, reminders, scan history) synced to Supabase cloud database |
...
|---|---|---|
| `GEMINI_API_KEY` | ✅ | Google Gemini API key for AI analysis, TTS, and Maps |
| `VITE_SUPABASE_URL` | ✅ | Supabase project URL |
| `VITE_SUPABASE_ANON_KEY` | ✅ | Supabase anonymous/public key (JWT format) |

> ⚠️ **Important:** The `.env` file is git-ignored and should never be committed. The `VITE_` prefix is required for Vite to expose variables to the client-side code via `import.meta.env`.
...
Go to your Supabase Dashboard → **SQL Editor** → **New Query**, then paste and run the contents of [`schema.sql`](schema.sql):
...
- `medications` / `reminders` → filtered by profile ownership
...
|---|---|---|
| Kore | Calm & Professional | English, Tamil |
| Puck | Friendly & Bright | Gujarati, Spanish |
| Charon | Clear & Formal | Telugu |
| Fenrir | Deep & Reassuring | Hindi, Punjabi |
| Zephyr | Gentle & Soft | Marathi, French |
...
class GeminiService {
  ...
  generateSpeech(text: string, voiceName: string): Promise<string>  // Returns Base64 PCM audio
}
```
...
<p align="center">
  Built with ❤️ using React, Google Gemini AI, and Supabase
</p>
]]>
